  ...
  def refresh
    ...
    if @mode == -1   # Main search screen
      ...
      when -3   # Cancel
        ...
        when 3, 4     # Height, weight
          self.x = @xstart + ((@index + 1) * @xgap * (@minmax % 2))
          self.y = @ystart + (@ygap * ((@minmax + 1) % 2))
        end
      end
    end
...
=end
  ...
  def pbRefreshDexList(index=0)
    ...
    when MODENUMERICAL
      ...
      dexlist[0] = nil if dexlist[0][5] && !$player.seen?(dexlist[0][:species])
  ...
  # to mimic the one used in BW
    textpos.push([GameData::Species.get(iconspecies).name,108,332,2,base,shadow]) if iconspecies
    if @searchResults
  ...
  def pbRefreshDexSearch(params,_index)
    ...
    textpos = [
       [_INTL("Search Mode"),Graphics.width/5,6,2,base,shadow],
       [_INTL("Order"),136,62,2,base,shadow],
       [_INTL("Name"),58,120,2,base,shadow],
       [_INTL("Type"),58,172,2,base,shadow],
       [_INTL("Height"),58,224,2,base,shadow],
       [_INTL("Weight"),58,276,2,base,shadow],
       [_INTL("Color"),326,120,2,base,shadow],
       [_INTL("Shape"),454,172,2,base,shadow],
       [_INTL("Reset"),80,344,2,base,shadow,1],
       [_INTL("Start"),Graphics.width/2,344,2,base,shadow,1],
       [_INTL("Cancel"),Graphics.width-80,344,2,base,shadow,1]
    ]
    ...
    # Draw type icons
    if params[2]>=0
      type_number = @typeCommands[params[2]].icon_position #was id_number
    ...
    if params[3]>=0
      type_number = @typeCommands[params[3]].icon_position #was id_number
      typerect = Rect.new(0,type_number*32,96,32)
    ...
    # Draw shape icon
    if params[9] >= 0
      shape_number = @shapeCommands[params[9]].icon_position #was id_number
